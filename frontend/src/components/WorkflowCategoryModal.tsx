...
export default function WorkflowCategoryModal({
  isOpen,
  onClose,
  category,
  onSaved,
}: WorkflowCategoryModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing && category) {
        await api.updateWorkflowCategory(category.id, { name: name.trim(), color });
      } else {
        await api.createWorkflowCategory({ name: name.trim(), color });
      }
      onSaved();
      handleClose();
    } catch (err) {
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Discovery, Strategy, Research"
